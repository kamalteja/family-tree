...
const VIEWER_TOOLBAR_IDS = [
  'principalSelector', 'memberCount', 'searchBtn', 'fitViewBtn',
  'toggleViewBtn', 'editModeBtn', 'kinshipBtn',
  'resetDataBtn', 'editJsonBtn', 'infoBtn',
];

function openKinshipPanel() {
  const rules = getKinshipRules();
  ...
  for (const id of VIEWER_TOOLBAR_IDS) {
...
function renderStatesIndex(rules) {
  ...
  for (const key of stateKeys) {
    ...
    if (isEditMode) {
      html += `<span class="kinship-card-edit" title="Edit state">&#9998;</span>`;
    }
    card.innerHTML = html;
...
function renderFocusedView(rules, stateKey) {
  ...
  const centerG = drawNode(svg, cx, cy, stateKey, rules.states[stateKey], true);
  ...
  for (let si = 0; si < satellitePositions.length; si++) {
    ...
    const g = drawNode(svg, sat.x, sat.y, sat.key, state, false);
